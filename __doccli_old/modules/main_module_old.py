...
from apis.docchi import get_series_list, get_episodes_count_for_serie, get_players_list, get_details_for_serie, get_skip_times
from subprocess import Popen, DEVNULL
from termcolor import colored
import webbrowser
from __doccli_old.apis.discord_old import update_rpc, set_running
...
def m_welcome():
    ...
    choices.append("Statystyki __doccli_old")
...
def m_settings():
    ...
    if res[0] == "Tak":
        clear()
        settings[0] = True
        choices2 = [{"type": "input", "message": "Wpisz co tylko zechcesz! Będzie to wyświetlane w II linijce statusu. Zostaw puste jeśli chcesz aby był wyświetlany domyślny status. [Minimalnie 2 znaki] (Domyślna wartość: 'Używa __doccli_old!') \n", "name": "status_dc"}]
...
def m_stats():
    ...
    print(colored("Używasz __doccli_old już od:", "white"), colored(delta_dt.days, "green"), colored("dni!", "white"))
    print(colored("Pierwsze uruchomienie __doccli_old:", "white"), colored(dt_c, "green"))
    print('')
...
PATH_config = os.path.join(PATH_home, ".config", "__doccli_old")
...
def load():
    ...
    if not os.path.exists(PATH_settings):
        with open(PATH_settings, 'w') as file:
            global settings
            settings = [True, "Używa __doccli_old!", True]
